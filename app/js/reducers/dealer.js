...
let initialState = {
  ...
  const isShop = localStorage.getItem('logintype') === 'SHOP' || false
  if (isDealer) {
    $.ajax({
    ...
    $.ajax({
      url: '/hcm/hcmShop/getBmcListByShopid',
      method: 'GET',
      dataType: 'JSON',
      async: false,
      success: (response) => {
        ...
        initialState.brandListToShop = data&&data.length !== 0 ? [...data] : [MOCK];
        initialState.currentBrandToShop =  data&&data.length !== 0 ?  data.filter(v => v.checked === 'true')[0] : MOCK;
      },
      error: (err) => {
        if (err.status === 503) {
...
const brandListToShop = (state = [], action) => {
  switch (action.type) {
    case type.GETBRANDLISTTOSHOP:
      return [...action.data]
      break;
...
const setCurrentBrandToShop = (state = {}, action) => {
  switch (action.type) {
    case type.SETCURRENTBRANDTOSHOP:
      return Object.assign({}, state, {
        ...action.data
      })
      break;
...
export default (state = initialState, action) => {
  return ({
    ...
    brandListToShop: brandListToShop(state.brandListToShop, action),
    currentBrandToShop: setCurrentBrandToShop(state.currentBrandToShop, action),
