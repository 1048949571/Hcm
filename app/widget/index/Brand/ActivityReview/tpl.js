...
const Tpl = ThatMain(that => {
  ...
  const { platformList, cacheActivityList, currentTabIndex, statusNums, pagination ,Identity} = that.state
        ...
        <Tabs activeKey={`${currentTabIndex}`} className="tabs" onChange={that.handleTabChange}>
          {
            TABS.map(v => (
              <TabPane tab={`${v.text}（${statusNums[v.nums] || 0}）`} key={v.key}>
                {
                  !!activityList.length?
                  activityList.map((value) => <ActivityListItem clicked={that.goToDetail} Identity={Identity} infos={value} key={value.id} />):
