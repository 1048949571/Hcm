...
class App extends React.Component {
    ...
    getInfo(){
        ...
        ajax.get('/hcm/hcmProduct/getById', {
            ...
            let {product_classify_id,bmcid,product_name,brand_name,floor_price,floor_transaction_price, ...addField} = response.data.data;
            this.setState({
    ...
    onSubmit(){
        let { id,name,itemid,type ,product_classify_id,bmcid,product_name,brand_name,floor_price,floor_transaction_price,addField,formatColumn} =this.state;
        ...
        if(!product_name||!(product_name.trim().length)){
            message.error((productName?productName[0].fieldname:'品名')+'不可为空')
            return
        }
        
        if(!floor_price||!(`${floor_price}`.trim().length)){
            message.error('最低标价不可为空')
            return
        }

        if(!floor_transaction_price||!(`${floor_transaction_price}`.trim().length)){
            message.error('最低成交价不可为空')
            return
        }
    ...
    render() {
        let { id, name, status, type, itemid ,product_classify_id,bmcid,product_name,brand_name,floor_price,floor_transaction_price,addField,formatColumn} =this.state;
            ...
            <div className='ActionPriceInfo'>
                ...
                <div className='content'>
                    <div className='box'>
                        ...
                        <div className='items'>
                        <div className='item'>
                                <span className='text red'>{'最低标价'}</span>
                                <Input maxLength='16' placeholder="请输入" onChange={e=>this.fixedIptChange(e,'floor_price')} value={floor_price} className='input'/>
                            </div>
                            <div className='item'>
                                <span className='text red'>{'最低成交价'}</span>
                                <Input maxLength='16' placeholder="请输入" onChange={e=>this.fixedIptChange(e,'floor_transaction_price')} value={floor_transaction_price} className='input'/>
                            </div>
                            {
                                price?
