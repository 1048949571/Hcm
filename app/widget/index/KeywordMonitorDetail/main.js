  ...
  constructor(props) {
    ...
    const taobaocolumns = [{
      ...
      render: (content, record, index) => (
        <span key={index}>
          {record.low_count}
          <a href={`/index.html#/monitorLowPriceScreenshot/${record.id}/${this.state.platform_code}`}>(低价截图)</a>
        </span>
      )
    }]
    const JDcolumns = [{
            ...
            if (this.functions(timeDate) < this.functions(res.data.data.key_times.split(",").sort()[i])) {
             // console.log(1)
              ...
              this.setState({
                ...
                this.list(this.state.platform_code, this.state.keyword_id, this.state.createtime, this.state.keytime, this.state.pageNo, this.state.pageSize)
