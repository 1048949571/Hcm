...
class JDInquiries extends React.Component {
    constructor(props) {
        ...
        const columns = [{
            ...
            dataIndex: 'tort_link',
            render: (content, record, index) => (
                <Popover placement="bottom" key={index} content={<div style={{ width: "200px", overflow: "hidden",height:"200px",overflowY:"scroll" }}>{record.tort_link!=null && record.tort_link!="" ? record.tort_link.replace("[",""):null}</div>} trigger="click">
                    <a style={{ height: "20px", width: "200px", display: "inline-block", overflow: "hidden" }}>{record.tort_link!=null && record.tort_link!="" ? record.tort_link.replace("[",""):null}</a>
                </Popover>
            )
        }, {
            title: '处理状态',
                        ...
                        record.state != null ? <span>
                            {
                                record.state == "完成" ? <span>
                                    <img src="../../../../img/icon_Login_prompt_succes.png" style={{ marginTop: "4px", float: "left" }} alt="" /><b>{record.state}</b>
                                        ...
                                        {
                                            record.state == "已撤诉" ? <span>
                                                <h4>{record.state}</h4>
                                                <span>撤诉人:{record.chesu_user}</span>
                                            </span> : record.state
                                        }
