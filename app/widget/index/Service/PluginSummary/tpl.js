            ...
            <FormItem label="平台" {...formItemLayout}>
              {
                getFieldDecorator('platform', {
                  ...
                  <Select getPopupContainer={trigger => trigger.parentNode}>
                    ...
                    {
                      platformList.map(v => <Option key={v.id} value={v.dictCode}>{v.dictName}</Option>)
                    }
                  </Select>
                )
              }
        ...
        return (
          <div>
            <p style={{display:'flex',alignItems:'center'}}>
              ...
              <span>{wangwang?'('+wangwang+')':null}</span>
              {
                authorize !== ''?
                <span style={{marginLeft:10}}>白名单</span>:
                null
              }
            </p>
            <p style={{display:'flex',alignItems:'center',marginTop:'3px'}}>
              <span>{
                platForm(platform,{verticalAlign: 'bottom',width:'20px',height:'20px'})
              }</span>
              &nbsp;&nbsp;&nbsp;
              <span>{
                arr.length?
