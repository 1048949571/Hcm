        ...
        >
            <div className='PlatformRetrieval'>
                ...
                <div className='content'>
                    {
                        that.state.PlatformRetrievalJS == "PlatformRetrievalJS" ? <div className="contentBox">
                            {
                                that.state.jingPinBrandList ? <ul>
                                    {
                                        that.state.jingPinBrandList.map((v, i) => {
                                            return <li key={i} style={that.state.bid == v.bid ? styleSX : styleSXx} onClick={() => that.JPList(v.bid)}>{v.name}</li>
                                        })
                                    }
                                </ul> : <li>请设置竞品商品名称</li>
                            }
                    ...
                    <div className='contentTop'>
                        ...
                        {`第  ${that.state.pageNo}  / ${Math.ceil(that.state.totalNum / that.state.pageSize)} 页`}
                            ...
                            <div style={{ margin: "20px" }}>
                                <a href={`/hcm/search/down_itemlist?type=${that.state.type}&title=${that.state.title}&pageSize=${that.state.pageSize}&pageNo=${that.state.pageNo}&brand_type=${that.state.brand_type}&bid=${that.state.bid}`}>
                                    <Button>下载数据</Button>
                                </a>
