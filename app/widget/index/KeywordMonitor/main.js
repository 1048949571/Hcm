...
class App extends React.Component {
  ...
  handleDeleteAll = () => {
    ...
    const data = {
      ...
      platform: platform_code===''?'':can.platform.find(v => v.dictCode === platform_code).dictName,
    }
    this.deleteFn(data)
  }
  ...
  handleShowKeywordInput = (flag, index) => {
    if (flag && this.state.dataList.some(v => v._isShowInput)) {
      Msg.warning('请先保存或取消编辑状态的条目！')
      return
    }
