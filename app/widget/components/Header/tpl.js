...
const isShop = localStorage.getItem('logintype') === 'SHOP';
...
const Tpl = ThatMain((that) => {
    ...
    let { collapsed, toggleCollapsed, brandList, currentBrand, brandListToKefu,brandListToShop,currentBrandToShop, location: { pathname } } = that.props;
    ...
    const defaultValueToShop = brandListToShop.length && brandListToShop.filter(v => v.checked === 'true')[0];
    let disabled = false;
    ...
    const SwitchBrandToShop = (
        <div className="switch-brand-wrapper">
            {
                brandListToShop.length === 0 ?
                    null :
                    <Select
                        showSearch
                        filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
                        defaultValue={defaultValueToShop.bmcid}
                        style={{ minWidth: '300px' }}
                        onChange={that.handleSwitchBrandToShop}
                        disabled={brandListToShop.length === 1 ? true : false}
                    >
                        {
                            brandListToShop.map(v => (
                                <Option
                                    key={v.bmcid}
                                    value={v.bmcid}
                                >
                ...
                that.state.Message_list.length > 0 ? <ul style={that.state.Message_list.length > 3 ? three : one}>
                    {
                        ...
                        所属分类：{
                                                                                        ...
                                                                                        </span> :  item.messagetype == "DPXZ" || item.messagetype == "DPSC"?
                                                                                        <span>
                                                                                            店铺名称：{JSON.parse(item.content).shop_name},旺旺：{JSON.parse(item.content).wangwang             }
                                                                                        </span> : null
                                            }</p>
        ...
        <div className='header'>
            ...
            <div className='right-user'>
                ...
                <div className='username'>
                    <span>{localStorage.realname&&localStorage.realname.substring(0, 1)}</span>
                    {localStorage.realname&&localStorage.realname}
                </div>
